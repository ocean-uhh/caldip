...
def test_ctd_reference_block_stated_once(results_dir, tmp_path):
    """The CTD sensor used is shown once in a CTD-reference block, not per row.

    castM5's detailed CSV carries a constant ``ctd_sensor_used`` column; the cast
    page lifts it into a "CTD reference" table and drops it from the repeated
    per-stop detail so the same value is not printed on every row.
    """
    out = tmp_path / "report"
    build_report(results_dir, out, cruise_name="msm142_2026")
    page = (out / "casts" / "castM5.html").read_text(encoding="utf-8")
    assert "<h2>CTD reference</h2>" in page
    assert "CTD sensor used" in page
    # The constant column is gone from the per-stop detail table.
    detail = page[page.index("Per-stop detail") :]
    assert "CTD sensor" not in detail


def test_pages_carry_generated_footer(results_dir, tmp_path):
    """Every report page ends with a caldip 'Generated by' footer with a date."""
    out = tmp_path / "report"
    build_report(results_dir, out, cruise_name="msm142_2026")
    for rel in ("index.html", "casts/castM7.html", "casts/castM7_inventory.html"):
        page = (out / rel).read_text(encoding="utf-8")
        assert "<footer>Generated by <strong>caldip</strong>" in page
        assert "UTC</footer>" in page
...
def test_cruise_recovered_from_netcdf(tmp_path):
    """A uniform cruise recorded in the netCDFs is the masthead label and a column."""
    src = tmp_path / "outputs"
    _copy_csvs(src)
    _write_cast_nc(src, {"castM7": "msm142_2026", "castM5": "msm142_2026"})
    out = tmp_path / "report"
    ...
    # The per-cast cruise is always shown as a column.
