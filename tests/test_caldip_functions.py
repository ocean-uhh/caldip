...
def test_calculate_universal_statistics_workflow():
    ...
    instruments = {
        '12345': {
            'data': xr.Dataset({
                'temp': ('time', np.full(1000, 15.0) + np.random.normal(0, 0.01, 1000)),
                'cond': ('time', np.full(1000, 35.0) + np.random.normal(0, 0.1, 1000))
            }, coords={'time': times}),
    ...
    reference_data = {
        'ctd': {
            'data': xr.Dataset({
                'prDM': ('time', pressure),
                'temp1': ('time', np.full(1000, 15.05) + np.random.normal(0, 0.005, 1000)),
                'c1mS/cm': ('time', np.full(1000, 35.05) + np.random.normal(0, 0.05, 1000))
            }, coords={'time': times}),
            'config': {}
        }
    }
