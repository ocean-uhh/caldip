...
def test_trim_to_deployment_basic():
    ...
    inst_out, ref_out = tools.trim_to_deployment(instruments, reference, config)
...
def test_trim_to_deployment_no_times():
    """Returns data unchanged when config has no deployment/recovery times."""
    ds = make_dataset(periods=100)
    instruments = {"S001": {"data": ds, "config": {}, "type": "sbe37"}}
    reference = {"ctd": {"data": ds.copy(), "config": {}}}

    inst_out, ref_out = tools.trim_to_deployment(instruments, reference, {})

    assert inst_out is instruments
    assert ref_out is reference


def test_trim_to_deployment_instrument_outside_window():
...
def test_extract_summary_picks_deepest_stop():
    ...
    summary = tools.summary_stats(df, {"name": "test"})
...
def test_extract_summary_empty_input():
    ...
    result = tools.summary_stats(pd.DataFrame(), {})
    assert result.empty
