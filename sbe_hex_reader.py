...
def _parse_hardware_data(header_text: str) -> Dict:
    ...
    if sensors_match:
        ...
        for sensor_match in sensor_matches:
            ...
            serial_match = re.search(r'<SerialNumber>(.*?)</SerialNumber>', sensor_content)
...
def _parse_config_data(header_text: str) -> Dict:
    ...
    pressure_installed = re.search(r'<PressureInstalled>(.*?)</PressureInstalled>', config_section)
...
def sbe37_hex_reader(hex_file: Union[str, Path]) -> xr.Dataset:
    ...
    for sensor_type, calib_data in calib_info['calibration'].items():
        ...
        if sensor_type == 'temperature' and 'temperature' in raw_data.columns:
            ...
            expected_press_coeffs = ['pa0', 'pa1', 'pa2', 'ptca0', 'ptca1', 'ptca2', 
                                   'ptcb0', 'ptcb1', 'ptcb2', 'ptempa0', 'ptempa1', 'ptempa2']
            filtered_coeffs = {k: v for k, v in coeffs.items() if k in expected_press_coeffs}
