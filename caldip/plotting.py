...
def create_universal_caldip_plot(
    ...
    for name, info in reference_data.items():
        ...
        for temp_var in ['t090C', 't190C', 'temp_primary', 'temp_secondary']:
            if temp_var in ds.data_vars:
                temperature_values.extend(ds[temp_var].values[~np.isnan(ds[temp_var].values)])
        ...
        if has_conductivity:
            for cond_var in ['c0mS/cm', 'c1mS/cm', 'cond_primary', 'cond_secondary']:
                if cond_var in ds.data_vars:
                    conductivity_values.extend(ds[cond_var].values[~np.isnan(ds[cond_var].values)])
            ...
            for var in cond_vars:
                if var in ds.data_vars:
                    fig.add_trace(go.Scatter(
                        ...
                        mode='lines', name=legend_name,
                        line=dict(color=color, width=2, dash=line_dash),
                        hoverinfo='skip', legendgroup=serial, showlegend=False
                    ), row=CONDUCTIVITY_ROW, col=1)
                    break
    ...
    for i, (name, info) in enumerate(reference_data.items()):
        ...
        if 'prDM' in ds.data_vars:
            fig.add_trace(go.Scatter(
                x=ds.time.values, y=ds.prDM.values,
                mode='lines', name=f'CTD {name}',
                line=dict(color=ref_color, width=3),
                hoverinfo='skip', legendgroup=f'ctd_{name}',
        ...
        for j, temp_var in enumerate(temp_sensors[:2]):  # Max 2 temperature sensors
            ...
            fig.add_trace(go.Scatter(
                x=ds.time.values, y=ds[temp_var].values,
                mode='lines', name=sensor_name,
                line=dict(color=sensor_color, width=3),
                hoverinfo='skip', legendgroup=f'ctd_t{j+1}',
        ...
        if has_conductivity:
            ...
            for j, cond_var in enumerate(cond_sensors[:2]):  # Max 2 conductivity sensors
                sensor_color = 'black' if j == 0 else 'gray'
                sensor_name = 'CTD C1' if j == 0 else 'CTD C2'
                
                fig.add_trace(go.Scatter(
                    x=ds.time.values, y=ds[cond_var].values,
                    mode='lines', name=sensor_name,
                    line=dict(color=sensor_color, width=3),
                    hoverinfo='skip', legendgroup=f'ctd_c{j+1}',
    ...
    if has_conductivity:
        fig.update_yaxes(range=conductivity_range, title_text="Conductivity (mS/cm)", row=CONDUCTIVITY_ROW, col=1)
    ...
    if show_bottle_stops and reference_data:
        ...
        if bottle_stops:
            ...
            for i, stop in enumerate(bottle_stops, 1):
                print(f"  Stop {i}: {stop['start_time']} to {stop['end_time']} "
                ...
                for row in range(1, n_rows + 1):
                    ...
                    fig.add_trace(go.Scatter(
                        x=[stop['start_time'], stop['start_time']], y=y_range,
                    ...
                    fig.add_trace(go.Scatter(
                        x=[stop['end_time'], stop['end_time']], y=y_range,
                    ...
                    fig.add_trace(go.Scatter(
                        x=[comp_start, comp_start], y=y_range,
                        mode='lines', line=dict(color='black', width=1, dash='dot'),
                        showlegend=False, hoverinfo='skip'
                    ), row=row, col=1)
                    
                    fig.add_trace(go.Scatter(
                        x=[comp_end, comp_end], y=y_range,
                        mode='lines', line=dict(color='black', width=1, dash='dot'),
                        showlegend=False, hoverinfo='skip'
                    ), row=row, col=1)
