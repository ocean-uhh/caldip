...
def parse_hex_header_sensors(hex_file: Union[str, Path]) -> Dict:
    ...
    try:
        ...
        cal_end = header_xml.find('</CalibrationCoefficients>') + len('</CalibrationCoefficients>')
            ...
            for calibration in root.findall('Calibration'):
                ...
                if sensor_id in ['temperature', 'conductivity', 'pressure', 'oxygen']:
                    ...
                    for child in calibration:
                        if child.tag in ['A0', 'A1', 'A2', 'A3']:  # Temperature coeffs
                        ...
                        elif child.tag in ['G', 'H', 'I', 'J', 'PCOR', 'TCOR', 'WBOTC']:  # Conductivity coeffs
                            # Map to seabirdscientific expected names
                            key_map = {'PCOR': 'cpcor', 'TCOR': 'ctcor', 'WBOTC': 'wbotc'}
                            ...
                            sensor_coeffs[child.tag.lower()] = float(child.text)
                        elif child.tag.startswith('OX') or child.tag in ['TAU20', 'NTAU']:  # Oxygen coeffs
                            sensor_coeffs[child.tag.lower()] = float(child.text)
...
def sbe37_hex_reader(hex_file: Union[str, Path]) -> xr.Dataset:
    ...
    if not enabled_sensors_list:
        ...
        if xmlcon_path.exists():
            ...
            raise ValueError(f"Could not determine sensor configuration for {hex_path}. No xmlcon file found and header parsing failed.")
    ...
    if calibration_coeffs or xmlcon_info:
        ...
        for sensor_id, sensor_info in sensor_configs.items():
            ...
            if sensor_type == 'temperature' and 'temperature' in raw_data.columns:
                ...
                temp_coeffs_filtered = {k: v for k, v in coeffs.items() 
                ...
                cond_coeffs_filtered = {k: v for k, v in coeffs.items() 
                                       if k in ['g', 'h', 'i', 'j', 'cpcor', 'ctcor', 'wbotc'] and isinstance(v, (int, float))}
                ...
                press_coeffs_filtered = {k: v for k, v in coeffs.items() 
                                        if k.startswith(('pa', 'ptc', 'ptemp')) and isinstance(v, (int, float))}
            ...
            # Handle oxygen data - apply SBE63 calibration to convert phase to ml/L
                ...
                try:
                    ...
                    from seabirdscientific.cal_coefficients import Oxygen63Coefficients, Thermistor63Coefficients
                    
                    # Create oxygen coefficients object
                    oxygen_coeffs_filtered = {
                        'a0': coeffs.get('oxa0', 0), 'a1': coeffs.get('oxa1', 0), 'a2': coeffs.get('oxa2', 0),
                        'b0': coeffs.get('oxb0', 0), 'b1': coeffs.get('oxb1', 0),
                        'c0': coeffs.get('oxc0', 0), 'c1': coeffs.get('oxc1', 0), 'c2': coeffs.get('oxc2', 0),
                        'e': coeffs.get('oxe', 0)
                    }
                    ...
                    therm_coeffs_filtered = {
                        'ta0': coeffs.get('oxta0', 0), 'ta1': coeffs.get('oxta1', 0),
                        'ta2': coeffs.get('oxta2', 0), 'ta3': coeffs.get('oxta3', 0)
                    }
                    therm_coefs = Thermistor63Coefficients(**therm_coeffs_filtered)
                    
                    # Get required data for conversion
                    oxygen_phase = raw_data['SBE63 oxygen phase'].values  # in microseconds
                    oxygen_temp = raw_data['SBE63 oxygen temperature'].values  # in degrees C
                    ...
                    if 'temp' in data_vars and 'cond' in data_vars and 'press' in data_vars:
                        ...
                        salinity_vals = np.full_like(pressure_vals, 35.0)  # Assume 35 PSU
                    ...
                    data_vars['oxygen_temp'] = ('time', raw_data['SBE63 oxygen temperature'].values)
    else:
